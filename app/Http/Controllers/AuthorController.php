...
class AuthorController extends Controller
{
    ...
    public function createPost(Request $request)
    {
        $request->validate([
            ...
            'post_content.required' => 'Post content cannot be empty.',
            'post_category.required' => 'Please select a valid category.',
            'featured_image.required' => 'A featured image is required.',
            'featured_image.mimes' => 'The image must be a file of type: jpeg, jpg, png.',
            'featured_image.max' => 'The image must not exceed 1MB.'
        ]);
        ...
        if ($request->hasFile('featured_image')) {
            ...
            if ($upload) {
                try {
                    // Create directories and thumbnails
                    ...
                    if ($saved) {
                        ...
                        return response()->json(['code' => 3, 'msg' => 'Failed to save post data.']);
                    }
                } catch (\Exception $e) {
                    return response()->json(['code' => 3, 'msg' => 'Error: ' . $e->getMessage()]);
                }
            }
            return response()->json(['code' => 3, 'msg' => 'Image upload failed.']);
        }
        return response()->json(['code' => 3, 'msg' => 'No featured image provided.']);
    }
