...
class AuthorForgotForm extends Component
{
    ...
    public function ForgotHandler()
    {
        ...
        $body_message = "We have received a request to reset the password for <b>Blognj</b> account associated with" .
            $this->email . "<br> You can reset your password by clicking the button below. ";
        $body_message .= '<br>';
        $body_message .= '<a href="' . $link . '" target="_blank"
                        style="color:#FFF;
                        border-color:#22bc66;
                        border-style:solid;
                        border-width:10px 180px;
        ...
        Mail::send('forgot-email-template', $data, function ($message) use ($user) {
            $message->from('noreply@mail.com', 'Blognj');
            $message->to($user->email, $user->name)
                ->subject('Reset Password');
        });

        $this->email = null;
        session()->flash('success', 'We have emailed your password your password reset link');
