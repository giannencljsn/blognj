            ...
            <div class="tab-pane fade" id="tabs-profile-17">
                <div>
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <h3>Set dark mode blog logo</h3>
                            <div class="mb-2" style="max-width: 200px">
                                <img src="{{ asset('back/dist/img/logo-favicon/' . \App\Models\Setting::find(1)->logo_dark) }}"
                                    alt="Dark Logo" class="img-thumbnail" id="logo-image-preview">

                            </div>
                            <form action="{{ route('author.change-dark-blog-logo') }}" method="post"
                                id="changeDarkBlogLogoForm">
                                @csrf
                                <div class="mb-2">
                                    <input type="file" name="logo_dark" class="form-control">
                                </div>
                                <button class="btn btn-primary">Change Dark logo</button>
                            </form>
                        </div>
...
@push('scripts')
    <script>
        ...
        $('#changeDarkBlogLogoForm').submit(function (e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 1) {
                        $('#logo-image-preview').attr('src', `/back/dist/img/logo-favicon/${response.msg}`);
                        alert('Dark logo updated successfully!');
                    } else {
                        alert('Failed to update the dark logo.');
                    }
                },
                error: function () {
